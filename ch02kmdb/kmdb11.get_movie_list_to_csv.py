...
service_key = 'c5cdfe0e1df87e2321819587c12daa89'
...
for thisYear in range(startYear, endYear):
    print('%s년도 크롤링 중입니다.' % thisYear)
    pageNumber = 1

    while True:
        movieDate = movieExtractor(pageNumber, pageSize, thisYear)
        #print(movieDate)

        try:
            totCnt = movieDate['movieListResult']['totCnt']
        ...
        if pageNumber == 1: #페이즈일 때만 전체 개수를 출력합니다.
        ...
        makeMovieTable(movieDate)

        if pageNumber == totCnt:
        # if pageNumber == 3:
            break
